...
def parse_and_transform(batch_id, input_, out_dir):
   ...
   try:
    ...
    print("sth wrong in parse_and_transform")
    return None

#==============================
import sys

def parse_and_transform2( input_, out_dir):
   print(input_, out_dir)
   batch_id=3
   try:
    out_loc = path.join(out_dir, '%d.txt' % batch_id)
    if path.exists(out_loc):
...
def main(in_loc, out_dir, n_workers=4, load_parses=False):
    ...
    #    jobs = partition(100, iter_comments(in_loc))
    #    print(jobs)
    #    do_work = parse_and_transform
        with io.open(in_loc, 'r', encoding='utf8') as file_:
	    parse_and_transform2(file_, out_dir)
